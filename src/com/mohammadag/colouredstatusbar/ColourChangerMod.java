...
import com.mohammadag.colouredstatusbar.hooks.oemhooks.NubiaHook;
...
public class ColourChangerMod implements IXposedHookLoadPackage, IXposedHookZygoteInit, IXposedHookInitPackageResources {
    ...
    public void handleLoadPackage(LoadPackageParam lpparam) throws Throwable {
        ...
        new NubiaHook(this, lpparam.classLoader);
    }
        ...
        try {
            if (mSystemIconViews != null) {
                for (ImageView view : mSystemIconViews) {
                    if (view != null) {
                        view.setColorFilter(new ColorMatrixColorFilter(getColorMatrix(iconTint)));
                    } else {
    ...
    private ColorMatrix getColorMatrix(int iconTint) {
        ColorMatrix cm = new ColorMatrix();
        if (iconTint != mSettingsHelper.getDefaultTint(Tint.ICON_INVERTED)) {
            cm.setSaturation(1);
        } else {
            cm.setSaturation(0);
            float[] array = new float[cm.getArray().length];
            System.arraycopy(cm.getArray(), 0, array, 0, cm.getArray().length);
        ...
        log("apply ColorMatrix: " + Arrays.toString(cm.getArray()));
        return cm;
    }
